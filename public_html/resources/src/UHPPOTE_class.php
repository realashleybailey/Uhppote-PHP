    ...
    public function procCmd($receive)
    {
        ...
        switch ($cmd) {
            ...
            case $this->command['search']:
                ...
                $index += 8;
                $ipex = str_split($ip, 2);
                $ip = implode('.', array_map(function ($el) {
                    return hexdec($el);
                }, $ipex));
                ...
                $mask = implode('.', array_map(function ($el) {
                    return hexdec($el);
                }, $mask));
                ...
                $gate = implode('.', array_map(function ($el) {
                    return hexdec($el);
                }, $gate));
